...
using System.IO;
using MessagePack;
using Decoders = MessagePack.Decoders;
namespace MessagePackCompat
{
    /// <summary>
...
namespace MessagePack.Decoders
{
    ...
    internal sealed class Bin8Bytes : IBytesDecoder
    {
        ...
        public byte[] Read(byte[] bytes, int offset, out int readSize)
        {
            ...
            {
                MessagePackCompat.MessagePackBinary.ThrowNotEnoughBytesException();
            }
    ...
    internal sealed class Float32Single : ISingleDecoder
    {
        ...
        {
            readSize = 5;
            return new MessagePackCompat.Float32Bits(bytes, offset + 1).Value;
        }
    }
    ...
    internal sealed class Float64Double : IDoubleDecoder
    {
        ...
        {
            readSize = 9;
            return new MessagePackCompat.Float64Bits(bytes, offset + 1).Value;
        }
    }
    ...
    internal sealed class FixString : IStringDecoder
    {
        ...
        public String Read(byte[] bytes, int offset, out int readSize)
        {
            var length = bytes[offset] & 0x1F;
            readSize = length + 1;
            return MessagePackCompat.StringEncoding.UTF8.GetString(bytes, offset + 1, length);
        }
    }
    ...
    internal sealed class Str8String : IStringDecoder
    {
        ...
        public String Read(byte[] bytes, int offset, out int readSize)
        {
            ...
            readSize = length + 2;
            return MessagePackCompat.StringEncoding.UTF8.GetString(bytes, offset + 2, length);
        }
    }
    ...
    internal sealed class Str16String : IStringDecoder
    {
        ...
        public String Read(byte[] bytes, int offset, out int readSize)
        {
            unchecked
            {
                var length = (bytes[offset + 1] << 8) + (bytes[offset + 2]);
                readSize = length + 3;
                return MessagePackCompat.StringEncoding.UTF8.GetString(bytes, offset + 3, length);
            }
        }
    }
    ...
    internal sealed class Str32String : IStringDecoder
    {
        ...
        public String Read(byte[] bytes, int offset, out int readSize)
        {
            unchecked
            {
                ...
                readSize = length + 5;
                return MessagePackCompat.StringEncoding.UTF8.GetString(bytes, offset + 5, length);
            }
        }
    }
    ...
    internal sealed class ReadNextMap : IReadNextDecoder
    {
        ...
        public int Read(byte[] bytes, int offset)
        {
            var startOffset = offset;
            int readSize;
            var length = MessagePackCompat.MessagePackBinary.ReadMapHeader(bytes, offset, out readSize);
            offset += readSize;
            for (int i = 0; i < length; i++)
            {
                offset += MessagePackCompat.MessagePackBinary.ReadNext(bytes, offset); // key
                offset += MessagePackCompat.MessagePackBinary.ReadNext(bytes, offset); // value
            }
            return offset - startOffset;
        }
    }
    ...
    internal sealed class ReadNextArray : IReadNextDecoder
    {
        ...
        public int Read(byte[] bytes, int offset)
        {
            var startOffset = offset;
            int readSize;
            var length = MessagePackCompat.MessagePackBinary.ReadArrayHeader(bytes, offset, out readSize);
            offset += readSize;
            for (int i = 0; i < length; i++)
            {
                offset += MessagePackCompat.MessagePackBinary.ReadNext(bytes, offset);
            }
            return offset - startOffset;
        }
    }
